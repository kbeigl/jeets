...
/**
 * Lookup Device transformed from Proto Device Message to Entity, add required fields and return
 * managed Entity for Persisting.
 */
@Converter
public final class DeviceEntityLookup {
  ...
  @Converter
  public static Device lookupDevice(NetworkDevice networkDevice, Exchange exchange)
      throws Exception {
    //      unwrap Device
    Device netDevice = networkDevice.getDevice();
    String uniqueId = netDevice.getUniqueid();
    LOG.info(
        "lookup incoming Device {} with {} positions and {} events.",
        uniqueId,
        netDevice.getPositions().size(),
        netDevice.getEvents().size());
  ...
  /**
   * Currently a new Device is added if uniqueID doesn't exist in database. This implies a Device
   * registration and should be changed, since Traccar Devices are registered via database (and
   * webfrontend).
   */
  private static Device findDeviceByUniqueId(Exchange exchange, final String uniqueId)
      throws Exception {
    ...
    return transactionTemplate.execute(
        new TransactionCallback<Device>() {
          public Device doInTransaction(TransactionStatus status) {
            entityManager.joinTransaction();
            //              TODO: return unique Device! not List.
            //              When you made changes on the managed objects, you do not worry about
            // merging
