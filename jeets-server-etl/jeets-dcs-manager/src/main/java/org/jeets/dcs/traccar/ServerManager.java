...
/**
 * This class is modeled after and replaces Traccar's ServerManager only with Spring and camel-netty
 * server management by composing URI Strings.
 *
 * <p>The org.traccar.ServerManager logic first reads all eligible *Protocol.classes. Then it
 * traverses the classes, checks for *name*Protocol entries &ltprotocolname.port> in Context.Config
 * to setup the Protocol server.
 *
 * <p>Currently the ServerManager starts Traccar Protocols, Jeets Protocols compiled into Traccar
 * and/or separately and a sample of a Netty En/Decoder.
 */
@Configuration
public class ServerManager implements BeanFactoryPostProcessor, EnvironmentAware {
   ...
   * i.e. @Bean definitions.
   *
   * <p>Spring boot internally uses Binder APIs to "map" the resolved properties into
   * the @ConfigurationProperties beans. This resolution happens during the spring boot startup
   * process AFTER the BeanFactoryPostProcessors get created. <br>
  ...
  public void postProcessBeanFactory(ConfigurableListableBeanFactory beanFactory)
    ...
    try {
      ...
      // keep running for non Traccar Servers inside DCS Manager ! exit in
      // protocols-traccar
    }
  ...
  /**
   * Setup Traccar servers with ports defined in the setup file and register them in Spring to be
   * handled by Camel and Netty/Starter.
   *
   * @param setupFile
  ...
  private void setupTraccarServers(ConfigurableListableBeanFactory beanFactory) throws Exception {
    ...
    LOG.info(
        "Setup {} Traccar servers in {} millis - ready for Camel to start!",
