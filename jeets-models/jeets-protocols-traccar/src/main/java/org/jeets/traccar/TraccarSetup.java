 ...
 *
 * <p>Each -protocols- project should supply a dedicated RouteBuilder with a Consumer Endpoint (DCS
 * output) which can be picked up by a DCS Manager.
 *
 * <p>The term '-Configured-' refers to the reduced traccar.xml file. It only holds servers with
 * additional testing material like protocols in the repos jeets-data/device.send folder. These
 * tests become part of the JeeTS build, test and integration test runs.
 */
public class TraccarSetup {
  ...
  public static Map<String, NettyServer> prepareServers(Map<Integer, Class<?>> protocolClasses) {
    ...
    if (protocolClasses.size() > 0) {
      for (int port : protocolClasses.keySet()) {
        ...
        if (port == 5040) continue;

        @SuppressWarnings("unchecked")
        Class<? extends BaseProtocol> protocolClass =
            (Class<? extends BaseProtocol>) protocolClasses.get(port);
      ...
      LOG.info(
          "added {} UDP servers and {} TCP servers (total: {})",
          udpServers,
          tcpServers,
          udpServers + tcpServers);
    } else {
  ...
  /**
   * Scan class path with ClassGraph and only return the classes configured in configFile. Scan is
   * restricted to org.traccar.BaseProtocol packages inside jeets- jars. (also verify for java -jar
   * execution!)
   *
   * <p>Note that ClassGraph is applied in a try .. resources block to handle ClassInfo/List/s in
   * it. The returned classes are loaded inside the method but have not been initialized yet!
   */
  public static Map<Integer, Class<?>> loadProtocolClasses() throws Exception {
    ...
    try (ScanResult result =
      ...
      LOG.info(
          "found {} BaseProtocol classes in {} millis",
          classInfos.size(),
      ...
      for (ClassInfo protocolClassInfo : classInfos) {
        ...
        if (port == -1) {
          //    				LOG.warn("port# for '{}' protocol is not defined in configuration file.",
          // protocolName);
        } else {
          /*
   ...
   *
   * <p>Prerequisite of calling this method is the {@link #contextInit} method on a configuration
   * file. If the port was not found in the Context port #-1 is returned and should be handled by
   * the caller. Else it will raise an Exception when trying to bind it: <br>
   ...
   * @param protocol
   * @return port#
   * @throws Exception if the &ltprotocol&gt.port was not read into the Context with {@link
   *     #contextInit} or is not defined in xml config file
   */
  public static int getProtocolPort(String protocol) throws Exception {
    ...
    if (isContextInitialized()) {
      ...
      throw new Exception(
          "Traccar Context was not initialized. "
  ...
  /**
   * Always apply this method to Initialize Traccar Context to ensure that it is only loaded once!
   * Method has public access in order to supply configFile from external environment and setup
   * traccar.Context only once.
   *
   * <p>Method returns fast, if context already is initialized. Therefore it doesn't harm to call it
   * multiple times.
   *
   ...
   *
   * <p>camel-netty and/or spring are/is tedious about localhost, which doesn't accept external
   * access (in ubuntu). On the remote system 0.0.0.0 should be used instead of 127.0.0.1.
   */
