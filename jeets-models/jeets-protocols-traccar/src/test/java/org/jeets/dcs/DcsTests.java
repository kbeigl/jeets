...
/**
 * These JUnit Tests should only validate basic functionality and -messages to detect problems
 * before running the dcs-manager. The dcs-itests should be used to test protocol sequences from
 * files.
 */
public class DcsTests extends CamelTestSupport {
  ...
  /**
   * These tests are based on the original traccar- and default.xml files with all available
   * protocols and ports.
   */
  @Test
  public void testAllServers() throws Exception {
    ...
    if (servers.size() > 0) {
      for (String protocolSpec : servers.keySet()) {
        ...
        //              String serverHost = (server.getAddress() == null) ? host :
        // server.getAddress();

        context.addRoutes(new TraccarRoute(uri, protocolSpec)); // id=teltonika-dcsRoute
        //              SpringBoot: @Bean(name = protocolSpec + "Route")
        //              beanFactory.registerSingleton(protocolSpec + "Route", new TraccarRoute(uri,
        // protocolSpec));
      }
    }
  ...
  /**
   * Individual server test with explicit *Protocol.class to ensure functionality of a single
   * protocol. The config files are not applied at all.
   */
  @Test
  ...
  /**
   * Hard coded setup with relative path and file must exist in the project to allow testing the
