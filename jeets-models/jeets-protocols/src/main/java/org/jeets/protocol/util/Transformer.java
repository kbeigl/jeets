...
/**
 * Transform Traccar Entities to binary Protobuffers for sending messages from client (Tracker) and
 * transform Protos back to Java Entities on server (DCS).
 *
 * <p>The Transformer (currently) is focused on Object Relations between Device, Positions and
 * Events. Note that their relations are modeled different in Persistence Unit and Protobuffer. In
 * the end both models allow many Events for one Position, i.e. n:1 The relation is unidirectional
 * in both models - only in different directions. <br>
 * Vice versa Events must have a Position to determine where and when the Event took place.
 * Therefore Events without Positions should be ignored or rather shouldn't be created in the first
 * place.
 *
 * <p>In the Protobuffer Model a Device can hold many positions and each position can hold many
 * events. Each event is accessed via the position parent.
 *
 * <p>In the Persistence Unit a Device can hold many positions and many events and the
 * event.position has to be looked up in the device.positions.
 */
public class Transformer {
   ...
   *
   * <p>see {@link Transformer} description for relevant Object Relations.
   *
   * @param deviceProto
   * @return
   */
   ...
   *
   * <p>Relation positionEntity.setDevice(dev) is not applied and should be handled externally. <br>
   * The Position Entity has no relation to its Events. Vice versa Event Entities are related to
   * Device and Position Entity.<br>
  ...
  /**
   * Transforms Event without relation to a position, i.e. Relations eventEntity.setPosition(pos)
   * and .setDevice(dev) are not applied and should be handled externally.
   *
   * @param eventProto
   * @return eventEntity
   */
  ...
  /**
   * This method should handle client events on server side. Server Events are not transmitted to
   * client.
   *
   * @param eventType
  ...
  /**
   * This method should handle server events and transform them to client events - if available.
   * Basically provided for development purposes.
   *
   * @param eventString
   * @return EventType
   */
