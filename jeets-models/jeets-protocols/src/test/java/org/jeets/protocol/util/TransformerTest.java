...
public class TransformerTest {
  ...
  public void transformEntityToProtoEvent() {
    ...
    assertEquals(
        eventEntity.getType(), Transformer.eventTypeProtoToEntityString(eventBuilder.getEvent()));
    assertEquals(
        eventBuilder.getEvent(), Transformer.stringToEventTypeProto(eventEntity.getType()));

    Jeets.Event eventProto = eventBuilder.build();
    assertEquals(
        eventEntity.getType(), Transformer.eventTypeProtoToEntityString(eventProto.getEvent()));
  ...
  public void transformEntityDeviceWithTwoPositionsWithEvent() {
    ...
    Device deviceEntity =
        org.jeets.model.traccar.util.Samples.createDeviceWithTwoPositionsWithEvent();
  ...
  public void transformEntityDeviceWithPositionWithTwoEvents() {
    ...
    Device deviceEntity =
        org.jeets.model.traccar.util.Samples.createDeviceWithPositionWithTwoEvents();
    //		System.out.println("deviceEntity (" + deviceEntity.getPositions().size() + " pos, "
    //  		+ deviceEntity.getEvents().size() + " evs) ");
  ...
  public void transformProtoDeviceWithPositionWithTwoEvents() {
    ...
    for (int ev = 0; ev < eventArray.length; ev++) {
      ...
      assertSame(
          "Event " + eventArray[ev].getType() + " has different Position Object than device!",
          deviceEntity.getPositions().get(0),
          eventArray[ev].getPosition());
    }
  }
