...
public class Notification implements Serializable {
	...
	public String toString() {
		final int maxLen = 3;
		return "Notification [id=" + id + ", always=" + always + ", attributes=" + attributes + ", notificators="
				+ notificators + ", type=" + type + ", devices="
				+ (devices != null ? devices.subList(0, Math.min(devices.size(), maxLen)) : null) + ", groups="
				+ (groups != null ? groups.subList(0, Math.min(groups.size(), maxLen)) : null) + ", calendar="
				+ calendar + ", users=" + (users != null ? users.subList(0, Math.min(users.size(), maxLen)) : null)
				+ "]";
	}
