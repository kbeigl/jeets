...
public class Geofence implements Serializable {
	...
	@ManyToMany
	@JoinTable(name = "tc_device_geofence", 
	...
	public String toString() {
		final int maxLen = 3;
		return "Geofence [id=" + id + ", area=" + area + ", attributes=" + attributes + ", description=" + description
				+ ", name=" + name + ", devices="
				+ (devices != null ? devices.subList(0, Math.min(devices.size(), maxLen)) : null) + ", calendar="
				+ calendar + ", groups=" + (groups != null ? groups.subList(0, Math.min(groups.size(), maxLen)) : null)
				+ ", users=" + (users != null ? users.subList(0, Math.min(users.size(), maxLen)) : null) + "]";
	}
