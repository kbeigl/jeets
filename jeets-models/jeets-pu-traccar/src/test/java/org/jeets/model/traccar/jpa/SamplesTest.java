...
/**
 * This test class is relying on an existing (test) database with a valid schema which is created in
 * the 'mvn test' cycle running the setup and tear down scripts via sql-maven-plugin to conserve
 * static scripts as an implicit regression test - in case of changes.
 *
 * <p>The same class could be executed a second time (search: junit execute test multiple times, see
 * JUnit @RepeatedTest) against a database dynamically created from hibernate.hbm2ddl.auto=create
 * (in setUpClass createEntityManagerFactory) to detect model differences between script- and ORM
 * generation.
 *
 * <p>Currently all tests run Entities directly against the EntityManager. -DAO and/or -Repository
 * constructs will require higher level testing. Transaction Management is also applied manually to
 * get a better understanding. In higher level components transactions can be added via Spring and
 * other automated solutions.
 *
 * <p>Implicitly all Samples should be tested.
 */
public class SamplesTest {
  ...
  */
  /**
   * The Device Entity related to Position and Event Entities form the ORM core for GPS tracking.
   * All atomic database interactions should be modeled in the JPA module before adding them on a
   * higher level.
   *
   * <p>This test is focused on repeated cascaded persists, no merge, no deletions which should not
   * necessarily be implemented in a GTS API.
   */
  @Test
   ...
   *
   * <p>Once you get any reference to any Object in the ORM you can reach every branch and leaf.
   * Test ensures that all existing uni/bidirectional relations are wired correctly.
   *
   * <p>Note that the relation Events-Position does not exist in database, but is modeled in the ORM
   * with event.getPosition()!
   */
  private void validateObjectRelations(List<Device> dbDevices) {
    ...
    for (int dev = 0; dev < dbDevices.size(); dev++) { // no order guaranteed
      ...
      assertTrue(
          "Last Position is NOT contained in Device-Positions!",
      ...
      switch (uniqueCount) {
        case 3: // DeviceWithTwoPositionsWithEvent 2/2
          for (int ev = 0; ev < events.size(); ev++) {
            assertTrue(
                "Event-Position is NOT contained in Device-Positions!",
        ...
        case 4: // DeviceWithPositionWithTwoEvents 1/2
          assertSame(
              "The Events do NOT point to the same Position!",
  ...
  /**
   * All Entities should have a .toString() method and the relations should be displayed
   * conservatively in order not to raise a StackOverflowError due to recursive .toString() Device >
   * Positions > Position > Device invocations.
   */
