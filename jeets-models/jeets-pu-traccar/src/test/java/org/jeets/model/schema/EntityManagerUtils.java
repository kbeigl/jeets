...
/**
 * This class provides different methods to create a schema in a database. Different methods can be
 * manually executed to run the standard tests against. The standard (especially DBUnit-) tests rely
 * on an existing, maybe populated database. The database is not cleaned or dropped after testing
 * and can be inspected with various sql tools like pgAdmin, h2console etc.
 *
 * <p>WARNING! Do not apply these methods to production database as it can be modified, deleted or
 * even dropped!
 *
 * <p>Note that these methods are expensive as each one has to create a new EntityManagerFactory
 * with individual sets of properties. Therefore they should not be (sequentially) invoked in the
 * regular mvn (integration) test cycle. A different approach would be to create individual
 * persistence units in persistence.xml: <br>
...
public class EntityManagerUtils {
  ...
  /**
   * The persistence unit supplies a complete ORM defined by related entity classes. The JeeTS
   * persistence units should also provide the database connection defined by driver, url, user and
   * password.
   */
  public static final String PERSISTENCE_UNIT_NAME = "jeets-pu-traccar-test";
  /**
   * The EntityManagerFactory provides the database connection to each new EntityManager and must be
   * static. As the EntityManager can not change its existing connection the EntityManagerFactory
   * needs to created for each method - which is too expensive for a regular test cycle.
   */
  private static EntityManagerFactory emFactory;

  /**
   * 'main method' to trigger private methods to manually create database and / or schema scripts.
   * After a database setup all tests can be run against it.
   */
  //	@Test
  ...
  /**
   * Hibernate feature to drop and create a database from the persistence units entity classes.
   *
  ...
  */
   ...
   *
   * <p>This test is not part of the 'mvn test' runs and should be manually applied, ONLY if the ORM
   * is changed, i.e. after version increments. The method will replace the drop and create scripts
   * applied by maven.
   *
   * <p>Make sure the existing scripts were not manually modified or add modifications to new
   * scripts! Also be aware that the new scripts will be appended to the existing ones and need to
   * cleaned accordingly.
   */
  ...
  /**
   * Apply hibernate proprietary DDL formatter to file: <br>
  ...
  private void logEntityManagerAndFactory(EntityManager em) {
    ...
    for (Map.Entry<String, Object> entry : props.entrySet()) {
      if (entry.getKey().startsWith("javax.persistence.")
          || entry.getKey().startsWith("hibernate."))
