...
import org.jeets.playback.gtfs.GtfsFactory;
...
//      create database access to GTFS model ----
        GtfsFactory gtfs = new GtfsFactory(dbUrl, dbUser, dbPassword);
...
//  private        List<Builder>  composeCourseTrack(List<CourseElement> courseElements) {
    private static List<Position> createPositionTrip(GtfsFactory gtfs, GtfsTrip trip) {
...
//      for (int pos = 0; pos < positions.size(); pos++) {
    ...
    private static Position findClosestPosition(GtfsStop stop, List<Position> positions) {
    ...
    private static Log logger = LogFactory.getLog(TransitTraffic.class);
