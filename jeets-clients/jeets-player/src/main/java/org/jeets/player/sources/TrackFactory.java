...
/**
 * This class was moved and reengineered in my-jeets-client as DatabaseFactory. Create a test case
 * with h2, insert mini track and replay to SampleReceiver as a template for a DBfactory...
 *
...
public class TrackFactory {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        positionEntities.size() + " positions retrieved " + "for device " + db.fromUniqueId);
   ...
   *
   * <p>The Fixtimes should not be sent from the past time and are therefore set to the time they
   * are actually played to listeners. <br>
  ...
  /** Immediately start submitting GPS Positions to Tracker according to real time stamp deltas. */
  @Deprecated
  private void play(List<Position> positions) {
    for (int pos = 0; pos < positions.size(); pos++) {
      ...
      if (pos < positions.size() - 1) { // exclude last position
        long nextPositionMs =
            positions.get(pos + 1).getFixtime().getTime() - position.getFixtime().getTime();
  ...
  private Date parseDate(String timestamp) {
    ...
    try {
      ...
      System.err.println(
          "Error parsing the Date String: " + timestamp + " to dataformat: " + dateFormat);
      //          usage(); DateFormat
      System.exit(1);
    }
    return date;
