...
/**
 * This Device is basically a wrapper for the camel-netty4 ProducerTemplate acting as the TCP client
 * interacting with the server.
 *
 * <p>This Device is focused on sending and receiving plain byte[] arrays, i.e. plain TCP
 * communication. The Raw Device can serve as a generic client to send (and receive) byte[] arrays
 * regardless of their proprietary protocol format.
 *
 * <p>Camel and ProducerTemplate are hidden in the Device, which can be handled as a regular Java
 * Object. Neither is Spring used for the Device.
 */
/* Developers Note
...
public class Device {
   ...
   *
   * <p>For one-way communication (isRequest=false) the send*Message methods will be queued and sent
   * whenever a connection is available. In request-response communication every message has to be
   * sent individually in order to process the ACK before sending the next message.
   *
   * @param host
   * @param port
   * @param isRequest
  ...
  /**
   * Without explicit {@link CamelContext} the first Device of an application creates a context for
   * all subsequent devices. By passing an existing context at creation of the first Device it will
   * be used for further devices.
   */
  public Device(String host, int port, boolean isRequest, CamelContext camelContext)
      throws Exception {
    this.host = host;
    this.port = port;
  ...
  private void setServerEndpoint(/* implies host, port */ ) {
    ...
    //         org.apache.camel.component.netty4.NettyComponent@9f46d94 as PropertiesComponent is
    // not in use  ???
  ...
  /**
   * As a first approach the first Device creates an internal static CamelContext and every
   * subsequent Device will use these.
   */
  ...
  /**
   * Convenience method to convert hex message to byte[] message and send it to the server.
   *
   * <p>see {@link #sendByteMessage} for details
   */
  ...
  /**
   * The 'send-' methods are 'request only' or 'fire and forget'. The caller sends a message and can
   * not expect a reply. Therefore the method returns void, i.e. continues or throws an Exception.
   *
   * <p>see {@link #asyncSendByteMessage} for details
   *
   * @param byteMessage
   * @throws Throwable
   */
  public byte[] sendByteMessage(byte[] byteMessage) throws Throwable {
    ...
    if (!isRequest) {
      ...
      String serverUri = "netty4:tcp://localhost:5046"
          ...
          //                  + "&disconnect=false"
          //                  + "&reuseChannel=true"
          //                  + "&requestTimeout=2000"    // [ms] handle with care! Used to
          // terminate test ONLY!
  ...
  */
   ...
   *
   * <p>In the real world a Tracker collects (GPS Unit) and sends (GSM Unit) messages separately. In
   * a tunnel usually both units can not operate. Therefore most devices have an internal storage
   * for unsent messages. The device should only remove messages after they have been sent to the
   * server, which can actually only be achieved with some Acknowledge message.
   */
  private BlockingQueue<byte[]> messageQueue = new LinkedBlockingQueue<byte[]>();

  /**
   * If transmission fails, try to reconnect after every {@link #reconnectTimeout} seconds to
  ...
  /**
   * All Messages are collected in a MessageQueue and the MessageLoop takes care of sending them
   * whenever connectivity allows it.
   */
   ...
   *
   * <p>The queue is restricted to a single Object, which can be the actual byte[] message or a
   * Throwable from the callback.
   *
   * <p>Note that the BlockingQueue was spontaneously chosen as 'some' blocking object. This could
   * be replaced with a dedicated blocking object for this very purpose. See Data class at
   * www.baeldung.com/java-wait-notify to mimic an Exchange with ex.in = message and ex.out =
   * Throwable or equals(ex.in) with SendMessageLoop = sender and SentCallback = receiver.
   */
  private BlockingQueue<Object> callbackQueue = new LinkedBlockingQueue<Object>(1);

  /**
   * Send a message as byte[] array, wait to establish connection and do not expect a reply.
   *
   * <p>This is the least secure communication, since the server does not acknowledge the reception
   * of a message. Vice versa the sender can not do its house keeping of removing sent messages.
   * Therefore a callback from Camel will inform about the result of sending the message: After
   * successful transmission the method will return, on failure it throws an Exception retrieved
   * from Netty.
   *
   * <p>Note that the 'send-' methods do not refer to TCP, i.e. Netty connections, but to Camel
   * Routes from netty producer to consumer (server). Camel's SynchronizationAdapter waits for the
   * completion and can access the Exchange in the onDone method to check, if the request was
   * successful (onComplete) or not (onFailure). This method is called from the MessageLoop Thread,
   * while the callback is caught in a different Thread.
   *
  ...
  private void asyncSendByteMessage(byte[] bytes) throws Throwable {
    ...
    try {
      ...
      if (returnValue.equals(bytes)) return;
      else // add typing to concrete Exceptions
      if (returnValue instanceof Throwable) throw (Throwable) returnValue;
  ...
  /**
   * This callback will put the returnValue in the callbackQueue, being the original byte[] message
   * to indicate success or a Throwable which can be handled externally. Since we don't expect a
   * return message it can be disposed.
   */
  ...
  /**
   * Currently this should be done explicitly for a graceful shutdown. It should be embedded in a
   * higher level context like org.apache.camel.main.Main being a command line tool for booting up a
   * CamelContext.
   */
