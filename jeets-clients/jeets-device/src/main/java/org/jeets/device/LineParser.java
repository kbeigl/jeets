...
/**
 * Parse different line formats to prepare 'device.send' method. Text line can be any source, i.e.
 * split lines from a text file or enter line to a stream input etc.
 */
public class LineParser {
  ...
  /**
   * File type One has four columns 'protocol host port sync hexmsg' separated by tabs. The textline
   * is parsed into the DeviceConfig, which is added as a header to be applied by the Device for
   * sending the hex message.
   */
  ...
  public void parseFileOne(Exchange exchange) throws Throwable {
    ...
    //      TODO Exception handling
    {
      ...
      if (attribs[4].endsWith("\r")) msg.setBody(attribs[4].substring(0, attribs[4].length() - 1));
      else msg.setBody(attribs[4]);
