...
** implemented jeets.proto/buffers as the JeeTS Protocol (compatible to Traccar Database Model)
** currently depends on `jeets-protocols-traccar` +
   to enable compiling additional protocols the Traccar way 

* generalized `jeets-dcs-traccar` to `jeets-dcs-mamager` +
...
( * restructure repository with jeets-traccar 
    and / or device-communication section ? )
