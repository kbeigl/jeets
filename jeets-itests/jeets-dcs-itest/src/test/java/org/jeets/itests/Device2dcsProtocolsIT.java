...
import static org.apache.camel.test.junit5.TestSupport.deleteDirectory; // deleteDirectory
...
public class Device2dcsProtocolsIT { // extends Camel/TestSupport
  ...
  public void testProtocolFiles() throws Exception {
    ...
    Assertions.assertTrue(deleteDirectory(deviceSentFolder));
    ...
    Assertions.assertEquals(testfiles.length, file.listFiles().length);
  ...
  private void testProtocolFile(String fileName) throws Exception, InterruptedException {
    ...
    Assertions.assertTrue(notify.matches(20, TimeUnit.SECONDS));
    ...
    Assertions.assertTrue(target.exists(), fileName + " was not sent!");
    Assertions.assertTrue(target.isFile(), fileName + " was not sent!");
